...
**1. A Text element whose content doesn't fit its fixed `%h` silently disappears** from the running page.
Not truncated — absent from the DOM. The editor still shows it and `run_issue_checker` reports nothing.
Two ways to trigger it:
- `%h` smaller than one rendered line (`ceil(%fs × %lh)`), e.g. a 20px heading in a 24px-tall element;
- text that **wraps** to more lines than `%h` allows — including a label that is 1–2px too wide for its
  box, such as a badge whose pill is snug around the word.

So over-estimate: allow `ceil(%fs × %lh) + 6` per line, and pad any width you estimated from a string
length. This is the easiest way to build a page that looks right in the editor and renders half-empty.
...
What does work on any app: the raw CSS properties **`font_family`** and **`font_weight`** (`"600"`) in `%p`.
But Bubble **quotes the whole `font_family` value**, so a CSS font *stack* becomes one invalid family name
and falls back silently — pass a **single family** that exists on the viewer's machine (`"Courier New"`,
`"Georgia"`), not `"ui-monospace, SFMono-Regular, monospace"`.
